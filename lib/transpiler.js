...
function addProperty(name, value, exprStmt) {
    var propAst = esprima.parse('({ '+name+': '+value+' })').body[0].expression.properties[0];
    exprStmt.expression.properties.push(propAst);
    return exprStmt;
}

function shimStyles(styles, elementName) {
    var shimmedStyles = styles,
        rules = [
            [/:host\(([^:]*):host\)\s*{/, elementName+'$1 {'],
            [/:host\(([^:]+)\)\s*{/, '$1 > '+elementName+' {'],
            [/:host(:hover|:active|:focus)\s*{/, elementName+'$1 {'],
            [/:host\s*{/, elementName+' {']
        ];
    
    rules.forEach(function(rule) {
        shimmedStyles = shimmedStyles.replace(rule[0], rule[1]);
    });

    return shimmedStyles;
}
...
function transpile(htmlString) {
    ...
    sourceAst = esprima.parse(mainScript, { loc: true });
    ...
    // If the component include a template, we push it as a property
    if (template) {
        template = "'"+template.replace(/\r?\n/g, '')+"'";
        addProperty('template', template, exprStmt);
    }
