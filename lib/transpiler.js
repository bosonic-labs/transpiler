...
var jstransform = require('jstransform'),
    ...
    transpileTemplate = require('./templates').transpileTemplate;
...
function visitProperty(traverse, node, path, state) {
    if (node.kind === 'get') {
        ...
        var key = node.key.name,
            setter = tools.searchSetter(path[0].properties, key);
        ...
        var key = node.key.name,
            getter = tools.searchGetter(path[0].properties, key);
        ...
        if (node.key.name === 'attributeChangedCallback' && state.injectAttributesCode) {
            utils.catchup(node.value.body.range[0] + '{'.length, state);
            utils.append('\n    if ([' + tools.stringifyArray(state.g.opts.attributes) + '].indexOf(name) !== -1) {\n', state);
            utils.append('      this[name + \'Changed\'].call(this, oldValue, newValue);\n', state);
            utils.append('    }', state);
        }
...
function visitObjectExpressions(traverse, node, path, state) {
    ...
    if (automaticTemplating) {
        if (type == 'js') {
            utils.append("var template = " + transpileTemplate(template) + "\n", state);
        } else {
    ...
    if (automaticTemplating) {
        var created = tools.searchMethod(node.expression.properties, 'createdCallback');
        if (created) {
            state = utils.updateState(state, {
    ...
    if (Array.isArray(attributes)) {
        var changed = tools.searchMethod(node.expression.properties, 'attributeChangedCallback');
        if (changed) {
            state = utils.updateState(state, {
                injectAttributesCode: true
            });
        } else {
            if (needsColon) {
                utils.append(',', state);
            ...
            utils.append('    if ([' + tools.stringifyArray(attributes) + '].indexOf(name) !== -1) {\n', state);
            utils.append('      this[name + \'Changed\'].call(this, oldValue, newValue);\n', state);
            utils.append('    }\n', state);
            utils.append('  } }', state);
            if (node.expression.properties.length > 0) {
...
function transpileScript(script, options, vanilla) {
    options = options || {};
